...
#define DAC_PRIORITY			configMAX_PRIORITIES - 0
#define I2S_PRIORITY			configMAX_PRIORITIES - 1
#define MIDI_PRIORITY			configMAX_PRIORITIES - 1
#define AUD_DEC_PRIORITY0		configMAX_PRIORITIES - 1
#define AUD_DEC_PRIORITY1		configMAX_PRIORITIES - 1
#define AUD_DEC_PRIORITY2		configMAX_PRIORITIES - 1
#define MIDI_DEC_PRIORITY		configMAX_PRIORITIES - 1
#define FS_PRIORITY				configMAX_PRIORITIES - 2
...
typedef struct
{
    INT8U   AudioCompressorInput;
    INT8U   AudioCompressorOutput;
    INT8U   AudioCompressorGain;
    INT8U   DAGC_en;
    INT8U   AutoMute_en;
    INT8U   AudioCompressor_en;
    INT8U   DAGC_Boost;
    INT8U   DAGC_Gain;
    //auto mute (noise reduction)
    INT8U   NR_SignalFrameThreshold;
    INT8U   NR_NoiseFrameThreshold;
    INT8U   NR_NoiseSuppressLevel;
    INT8U   NR_SignalCountThreshold;
    INT8U   NR_FastReleaseThreshold;
    INT8U   NR_FastReleaseEnable;
    INT16U  NR_SignalThreshold;
    INT8U   NR_FastRampUpEnable;
    INT8U   NR_MuteFisrtEnable;
    INT16U  NR_MuteFisrtCount;
    INT8U   NR_AdjustPGA_Enable;
    INT8U   NR_SuppressSpeed;
    INT8U   NR_PGAHoldReleaseEnable;
    INT16U  NR_PGAHoldCount;
    INT16U  NR_PGAReleaseCount;
    INT8U   NR_SignalThresholdAutoAdjust;
    //DAGC
    INT16U  DAGC_Threshold;
    INT16U  DAGC_UpdateRrequency;
    INT16U  DAGC_UpdateThreshold;
    INT8U   DAGC_AttackScale;
    INT8U   DAGC_AttackTime;
    INT8U   DAGC_ReleaseScale;
    INT8U   DAGC_ReleaseTime;
} AUDIO_GAIN_CONFIG;
...
typedef struct
{
    ...
    INT8U     Main_Channel;
    INT32U    i2s_channel;
    AUDIO_GAIN_CONFIG audio_config;
} STAudioTaskPara;
...
typedef struct
{
    ...
    struct User                         //Source File handle and memory address
    {
        ...
        INT32S FileHandle_2;            //File Number by File System or user Define for 2 sensor
...
typedef struct {
        INT8U       OutputGrayScale;     /*1: output gray scale; 0: output the original color format*/
		INT8U       *OutputBufPtr;
...
typedef enum
{
		...
		IMAGE_ENCODE_OUTPUT_FORMAT_YUV400
...
typedef struct {
		...
		INT32U		CsiDramMode;		//0: csi with pscaler mode, 1: csi with dram mode
		...
		INT16U		Target2Width;		//video encode use
		INT16U		Target2Height;		//video encode use
		INT16U      SensorWidth;        //video encode use
		INT16U      SensorHeight;       //video encode use
		INT16U      Sensor2Width;        //video encode use
		INT16U      Sensor2Height;       //video encode use
		INT16U      DisplayWidth;
		...
		INT32U 		AudChannel;			//for avi encode only
		IMAGE_OUTPUT_FORMAT	OutputFormat;
		IMAGE_OUTPUT_FORMAT	EncodeFormat;
		//BinmodeConfig
		//binmode method bit[5:4] 0(AVG) 1(SKIP) 2(BYPASS)
        //binmode scale bit8 0:BY2, 1:BY4
        //binmode crop_en bit12
        //binmode bitmode bit14 0:8bit, 1:10bit
		INT32U      BinmodeConfig;
		INT32U      BinmodeConfig2;
} VIDEO_ARGUMENT;
