...
#if APP_WAV_DECODE_EN == 1 || APP_MP3_DECODE_EN == 1 || APP_A1800_DECODE_EN == 1 || APP_A6400_DECODE_EN == 1 || APP_OGG_DECODE_EN == 1
...
#if NAND_APP_EN == 1
extern INT32S  g_audio_data_offset;
#endif
INT32S audio_encoded_data_read(INT32U buf_addr, INT32U buf_size, INT8U *data_start_addr, INT32U buf_offset)
{
	...
	if(g_user_read_data_mode == 6)////植SDRAM笢畦AUDIO杅擂
	{
...
INT32U Snd_GetLen(void)	//read file length
{
	struct stat_t state_buffer;
 	...
 	fs_fstat(G_Snd_UserFd, &state_buffer);
...
INT32S Snd_Lseek(INT32S offset,INT16S fromwhere)
{
	...
	//Just for demo use
    fs_lseek(G_Snd_UserFd, offset, SEEK_CUR);
	R_Offset = fs_lseek(G_Snd_UserFd, 0, SEEK_CUR);
	return R_Offset;
}
