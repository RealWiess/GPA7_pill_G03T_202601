 ...
 **********************/
    ...
    static bool img_src_cache_compare(const void * src, const lv_img_decoder_dsc_t * dec_dsc);
#endif
...
_lv_img_cache_entry_t * _lv_img_cache_open(const void * src, lv_color_t color, int32_t frame_id)
{
        ...
        if(color.full == cache[i].dec_dsc.color.full &&
           ...
           img_src_cache_compare(src, &(cache[i].dec_dsc)) ) {
            ...
            if(cached_src->life > LV_IMG_CACHE_LIFE_LIMIT)
                cached_src->life = LV_IMG_CACHE_LIFE_LIMIT;//1000
            //DBG_PRINT("++ Cache Hit:I(%d)>life(%d) csrc(0x%x)",i,cache[i].life,cached_src);
            //DBG_PRINT("H ");
            break;
        }
    }
    ...
    if(cached_src) {
        //DBG_PRINT(" hit end return here csr:<%s>img(%x) ++\r\n",cached_src->dec_dsc.src,cached_src->dec_dsc.img_data);
        return cached_src;
    }
    ...
    for(i = 1; i < entry_cnt; i++) {
        //DBG_PRINT("cl[%d][%d]:csl[%d]\r\n",i,cache[i].life , cached_src->life);
        if(cache[i].life < cached_src->life) {
            cached_src = &cache[i];
            //DBG_PRINT("==Cache Miss:I(%d)>life(%d)src(%s)\r\n",i,cache[i].life,cached_src->dec_dsc.src);
        }
    }
    ...
    if(cached_src->dec_dsc.src) {
        ...
        //DBG_PRINT("Cache miss, close entry %s ==\r\n",cached_src->dec_dsc.src);
    }
    else {
        //DBG_PRINT("Cache miss, cached empty %s ==\r\n",cached_src->dec_dsc.src);
    }
#else
    ...
    if(open_res == LV_RES_INV) {
        DBG_PRINT("Image draw cannot open the image resource");
    ...
    //DBG_PRINT("-- CO_MS_rtn:%s calf:%d tto:%d csrc:%x --\r\n\n",cached_src->dec_dsc.src,cached_src->life,cached_src->dec_dsc.time_to_open,cached_src);
    return cached_src;
}
...
/**
 * Compare the source content with the cached content
 * If they are the same, return true, otherwise return false
 * Only compare when src_type is LV_IMG_SRC_VARIABLE
 */
static bool img_src_cache_compare(const void * src, const lv_img_decoder_dsc_t * dec_dsc)
{
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        return (img_dsc->data == dec_dsc->img_data) &&
                (img_dsc->header.cf == dec_dsc->header.cf) &&
                (img_dsc->header.w == dec_dsc->header.w) &&
                (img_dsc->header.h == dec_dsc->header.h);
    }
