...
extern void asmmemset2(void* dest, lv_color16_t v, int len);
extern void asmmixpremult(uint16_t *dest_buf, uint16_t *color_premult, uint8_t opa_inv, uint16_t width);
extern void asmcolormix(uint16_t *dest_buf, uint16_t *src_buf, uint8_t opa, uint16_t width);
/**********************
 *  STATIC VARIABLES
 **********************/
    ...
    if(mask == NULL) {
        if(opa >= LV_OPA_MAX) {
            ...
            for(y = 0; y < h; y++) {
                ...
                asmmemset2(dest_buf, color, w); /* generalplus */
                dest_buf += dest_stride;
            }
        #else
            /* generalplus */
        #if LV_COLOR_DEPTH == 16
            uint8_t c[2] = {color.full, (uint16_t)color.full>>8};
        #elif LV_COLOR_DEPTH == 32
            uint8_t c[4] = {color.full, (uint32_t)color.full>>8, (uint32_t)color.full>>16, (uint32_t)color.full>>24};
        #endif
            uint8_t pixel_bytes = LV_COLOR_DEPTH / 8;
            for(x=0; x<w; x++) {
                /* LV_COLOR_DEPTH == 16, lv_color_t -> lv_color16_t (lv_color.h) */
                asmmemcpy(dest_buf+x, c, pixel_bytes);
            }
            for(y=1;y<h;y++)
            {
                asmmemcpy(dest_buf+y*dest_stride, dest_buf, w*pixel_bytes);
            }
        #endif
        }
        /*Has opacity*/
            ...
            #else
                asmmixpremult((uint16_t *)dest_buf, color_premult, opa_inv, w);
            #endif
                dest_buf += dest_stride;
            }
        }
...

{
    ...
    if(mask == NULL) {
        if(opa >= LV_OPA_MAX) {
            ...
            #else
                asmcolormix((uint16_t*)dest_buf, (uint16_t*)src_buf, opa, w);
            #endif
                dest_buf += dest_stride;
                src_buf += src_stride;
            }
