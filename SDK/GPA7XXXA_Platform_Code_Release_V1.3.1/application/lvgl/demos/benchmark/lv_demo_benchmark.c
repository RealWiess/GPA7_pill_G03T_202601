...
void lv_demo_benchmark_run_scene(int_fast16_t scene_no)
{
    ...
    if(scenes[scene_act].create_cb) {
        lv_label_set_text_fmt(title, "%"LV_PRId32"/%"LV_PRId32": %s%s", (INT32S)scene_act * 2 + (opa_mode ? 1 : 0),
                              (INT32S)(dimof(scenes) * 2) - 2,
...
static void generate_report(void)
{
    ...
    lv_label_set_text_fmt(title, "Weighted FPS: %"LV_PRIu32, (INT32U)fps_weighted);
    DBG_PRINT("Weighted FPS: %u\r\n", fps_weighted);

    subtitle = lv_label_create(lv_scr_act());
    lv_label_set_text_fmt(subtitle, "Opa. speed: %"LV_PRIu32"%%", (INT32U)opa_speed_pct);
    DBG_PRINT("Opa. speed: %u%%\r\n", opa_speed_pct);
    ...
    for(i = 0; scenes[i].create_cb; i++) {
        ...
        if(scenes[i].fps_normal < 20 && scenes[i].weight >= 10) {
            ...
            lv_snprintf(buf, sizeof(buf), "%"LV_PRIu32, (INT32U)scenes[i].fps_normal);
        ...
        if(scenes[i].fps_opa < 20 && LV_MAX(scenes[i].weight / 2, 1) >= 10) {
            ...
            lv_snprintf(buf, sizeof(buf), "%"LV_PRIu32, (INT32U)scenes[i].fps_opa);
            lv_table_set_cell_value(table, row, 1, buf);
...
static void report_cb(lv_timer_t * timer)
{
    ...
    if(opa_mode) {
        ...
        lv_label_set_text_fmt(subtitle, "Result : %"LV_PRId32" FPS",
                              (INT32S)scenes[scene_act].fps_opa);
        ...
        lv_label_set_text_fmt(subtitle, "Result : %"LV_PRId32" FPS",
                              (INT32S)scenes[scene_act].fps_normal);
...
static void scene_next_task_cb(lv_timer_t * timer)
{
    ...
    if(scenes[scene_act].create_cb) {
        lv_label_set_text_fmt(title, "%"LV_PRId32"/%"LV_PRId32": %s%s", (INT32S)scene_act * 2 + (opa_mode ? 1 : 0),
                              (INT32S)(dimof(scenes) * 2) - 2,  scenes[scene_act].name, opa_mode ? " + opa" : "");
        if(opa_mode) {
            lv_label_set_text_fmt(subtitle, "Result of \"%s\": %"LV_PRId32" FPS", scenes[scene_act].name,
                                  (INT32U)scenes[scene_act].fps_normal);
            DBG_PRINT("%d. Result of \"%s\"\t: %d FPS\r\n", scene_act * 2 + 1, scenes[scene_act].name, scenes[scene_act].fps_normal);
        }
        else {
            if(scene_act > 0) {
                lv_label_set_text_fmt(subtitle, "Result of \"%s + opa\": %"LV_PRId32" FPS", scenes[scene_act - 1].name,
                                      (INT32U)scenes[scene_act - 1].fps_opa);
                DBG_PRINT("%d. Result of \"%s + opa\"\t: %d FPS\r\n", scene_act * 2, scenes[scene_act - 1].name, scenes[scene_act - 1].fps_opa);
            }
            else {
                lv_label_set_text(subtitle, "");
            }
