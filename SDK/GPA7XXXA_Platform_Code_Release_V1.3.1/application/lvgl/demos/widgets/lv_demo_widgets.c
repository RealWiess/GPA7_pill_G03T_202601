    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {
        ...
        *s = LV_MAX(*s, 60);
        ...
        if(dsc->part == LV_PART_KNOB && lv_obj_has_state(obj, LV_STATE_PRESSED)) {
            char buf[8];
            lv_snprintf(buf, sizeof(buf), "%"LV_PRId32, (INT32S)lv_slider_get_value(obj));
    ...
    if(code == LV_EVENT_PRESSED || code == LV_EVENT_RELEASED) {
        ...
        /*Add the faded area before the lines are drawn */
            ...
            if(lv_chart_get_pressed_point(obj) == dsc->id) {
                ...
                char buf[8];
                lv_snprintf(buf, sizeof(buf), "%"LV_PRIu32, (INT32U)dsc->value);
...
static void meter1_indic1_anim_cb(void * var, int32_t v)
{
    ...
    lv_label_set_text_fmt(label, "Revenue: %"LV_PRId32" %%", (INT32S)v);
}
...
static void meter1_indic2_anim_cb(void * var, int32_t v)
{
    ...
    lv_label_set_text_fmt(label, "Sales: %"LV_PRId32" %%", (INT32S)v);
...
static void meter1_indic3_anim_cb(void * var, int32_t v)
{
    ...
    lv_label_set_text_fmt(label, "Costs: %"LV_PRId32" %%", (INT32S)v);
}
...
static void meter2_timer_cb(lv_timer_t * timer)
{
    ...
    lv_label_set_text_fmt(label, "Desktop: %"LV_PRIu32, (INT32U)session_desktop);

    label = lv_obj_get_child(card, -3);
    lv_label_set_text_fmt(label, "Tablet: %"LV_PRIu32, (INT32U)session_tablet);

    label = lv_obj_get_child(card, -1);
    lv_label_set_text_fmt(label, "Mobile: %"LV_PRIu32, (INT32U)session_mobile);
}
...
static void meter3_anim_cb(void * var, int32_t v)
{
    ...
    lv_label_set_text_fmt(label, "%"LV_PRId32, (INT32S)v);
}
